---
title: "一文看透 Kotlin 协程本质"
source: "https://juejin.cn/post/6987724340775108622?from=search-suggest"
author:
  - "[[苏火火]]"
published: 2021-07-22
created: 2026-04-28
description: "前言：勤学如春起之苗，不见其增，日有所长。——陶潜前言 公司开启新项目了，想着准备亮一手 Kotlin 协程应用到项目中去，之前有对 Kotlin 协程的知识进行一定量的学习，以为自己理解协程了，结果"
tags:
  - "clippings"
---
![src=http___ac-q.static.booking.cn_images_hotel_max1280x900_166_166698539.jpg&refer=http___ac-q.static.booking.jfif](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/42bdd41ab2da4f5faccaae6f7586afdd~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1200&h=800&s=140135&e=jpg&b=201d19)

> 前言：勤学如春起之苗，不见其增，日有所长。           ——陶潜

## 前言

公司开启新项目了，想着准备亮一手 Kotlin 协程应用到项目中去，之前有对 Kotlin 协程的知识进行一定量的学习，以为自己理解协程了，结果……实在拿不出手！

![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/98780494a12447c581e890a70f25a3d9~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=400&h=400&s=154402&e=png&b=f9f9f9)

为了更好的加深记忆和理解，更全面系统深入地学习 Kotlin 协程的知识，协程将分为三部分来讲解，本文是第一篇：

> Kotlin 协程实战进阶（一、筑基篇）
> 
> [Kotlin 协程实战进阶（二、进阶篇）](https://juejin.cn/post/6992629783674748936 "https://juejin.cn/post/6992629783674748936")
> 
> [Kotlin 协程实战进阶（三、原理篇）](https://juejin.cn/post/7143386748783968292 "https://juejin.cn/post/7143386748783968292")

## 本文大纲

![Kotlin协程.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/71c2de91c46349aa9808a8786051d2a4~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1650&h=6574&s=670705&e=png&b=ffffff)
...
![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c25b2ea4193340a1949bd5643ddeadd1~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=3546&h=1706&s=2370234&e=png&b=fdfbfb)
...
```kotlin
体验AI代码助手 代码解读复制代码    val call: Call<User> = userApi.getUserInfo("suming")
...
```kotlin
体验AI代码助手 代码解读复制代码GlobalScope.launch(Dispatchers.Main) {//开始协程：主线程
...
![436f83cbae9f407db24538c0922b6adc_tplv-k3u1fbpfcp-watermark.gif](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3d187753eb754980bde6be515c7fc915~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=720&h=405&s=2115073&e=gif&f=100&b=2a2a2a)

[此动图来源](https://juejin.cn/post/6883652600462327821 "https://juejin.cn/post/6883652600462327821")
...
```kotlin
体验AI代码助手 代码解读复制代码fun <T> runBlocking(context: CoroutineContext = EmptyCoroutineContext, block: suspend CoroutineScope.() -> T): T
```
...
```kotlin
体验AI代码助手 代码解读复制代码fun runBloTest() {
    print("start")
...
![runBlocking.gif](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/accf412e1c044b1e8673788e1240f197~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=153&s=2631&e=gif&f=3&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码public fun CoroutineScope.launch(
...
```kotlin
体验AI代码助手 代码解读复制代码fun launchTest() {
    print("start")
...
![launch1.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/fb4515d8f4114237a511443624a5aac2~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=153&s=3210&e=gif&f=3&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码fun launchTest2() {
    print("start")
...
![launch2.gif](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2d61752dc3c1439999bb742bcb3affc7~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=155&s=3242&e=gif&f=3&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码fun launchTest3() {
    print("start")
    GlobalScope.launch {
        delay(1000)
        print("CoroutineScope.launch")
...
![launch3.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/7c71bb87907e487f8f798c4c814d0e9f~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=155&s=4530&e=gif&f=4&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码public fun <T> CoroutineScope.async(
    context: CoroutineContext = EmptyCoroutineContext,
    start: CoroutineStart = CoroutineStart.DEFAULT,
...
![async1.gif](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3512f6bfaebc409f9f9ad15af103c392~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=153&s=4307&e=gif&f=3&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码fun asyncTest2() {
    print("start")
    GlobalScope.launch {
...
![async2.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/53ab2a9c84a44d5b8a938eb4e8054368~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=197&s=7735&e=gif&f=5&b=2b2b2b) 上面的代码就是一个简单的并发示例， `async` 是不阻塞线程的，也就是说上面三个 `async{}` 异步任务是同时进行的。通过 `await()` 方法可以拿到 `async` 协程的执行结果，可以看到两个协程的总耗时是远少于9秒的，总耗时基本等于耗时最长的协程。
...
```kotlin
体验AI代码助手 代码解读复制代码public interface Job : CoroutineContext.Element {
    //活跃的，是否仍在执行
...
```kotlin
体验AI代码助手 代码解读复制代码fun jobTest() = runBlocking {
...
![join.gif](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2874b935fc6e419fb7590016f73c04cf~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=779&h=174&s=6512&e=gif&f=5&b=2b2b2b)
...
```kotlin
体验AI代码助手 代码解读复制代码public interface Deferred<out T> : Job {
    //等待协程执行完成并获取结果
...
```kotlin
体验AI代码助手 代码解读复制代码public interface CoroutineScope {
...
```kotlin
体验AI代码助手 代码解读复制代码fun scopeTest() {
    //创建一个根协程
...
```kotlin
体验AI代码助手 代码解读复制代码class MainActivity : AppCompatActivity() {
...
```kotlin
体验AI代码助手 代码解读复制代码class MainViewModel : ViewModel() {
    fun getData() {
...
```kotlin
体验AI代码助手 代码解读复制代码public abstract class CoroutineDispatcher :
...
```kotlin
体验AI代码助手 代码解读复制代码fun dispatchersTest() {
    //创建一个在主线程执行的协程作用域
    val mainScope = MainScope()
    mainScope.launch {
...
```kotlin
体验AI代码助手 代码解读复制代码//用给定的协程上下文调用指定的挂起块，挂起直到它完成，并返回结果。
...
```kotlin
体验AI代码助手 代码解读复制代码//协程的持久上下文。它是[Element]实例的索引集,这个集合中的每个元素都有一个唯一的[Key]。
...
```kotlin
体验AI代码助手 代码解读复制代码//用户指定的协程名称。此名称用于调试模式。
...
```kotlin
体验AI代码助手 代码解读复制代码GlobalScope.launch(CoroutineName("GlobalScope")) {
...
```kotlin
体验AI代码助手 代码解读复制代码[DefaultDispatcher-worker-2] CoroutineName(CoroutineA)
```
...
```kotlin
体验AI代码助手 代码解读复制代码GlobalScope.launch {
    //通过+号运算添加多个上下文元素
...
```kotlin
体验AI代码助手 代码解读复制代码context == [CoroutineName(协程1), Dispatchers.Main]
...
![image.png](https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1486fd1c001e413796321a6cd666b4b5~tplv-k3u1fbpfcp-jj-mark:3024:0:0:0:q75.awebp#?w=1093&h=102&s=89041&e=png&b=2b2b2a)
...
```kotlin
体验AI代码助手 代码解读复制代码//Continuation接口表示挂起点之后的延续，该挂起点返回类型为“T”的值。
...
```java
体验AI代码助手 代码解读复制代码public interface Callback {
  //请求失败回调
...
```kotlin
体验AI代码助手 代码解读复制代码@GET("users/{login}")
...
```kotlin
体验AI代码助手 代码解读复制代码public abstract getUserSuspend(Ljava/lang/String;Lkotlin/coroutines/Continuation;)Ljava/lang/Object;
```
...
```kotlin
体验AI代码助手 代码解读复制代码public interface Continuation<in T> {
    //协程上下文
    public val context: CoroutineContext
...
```kotlin
体验AI代码助手 代码解读复制代码public fun CoroutineScope.launch(): Job {
...
```kotlin
体验AI代码助手 代码解读复制代码public fun <R> start(start: CoroutineStart, receiver: R, block: suspend R.() -> T) {
    initParentJob()
...
```kotlin
体验AI代码助手 代码解读复制代码public operator fun <T> invoke(block: suspend () -> T, completion: Continuation<T>): Unit =
   when (this) {
...
```kotlin
体验AI代码助手 代码解读复制代码public fun <T> (suspend () -> T).startCoroutine(completion: Continuation<T>) {
...
- [Kotlin官网](https://link.juejin.cn/?target=https%3A%2F%2Fdeveloper.android.google.cn%2Fkotlin%2Fcoroutines "https://developer.android.google.cn/kotlin/coroutines")
- 《深入理解Kotlin协程》
- [慕课网之《新版Kotlin从入门到精通》](https://link.juejin.cn/?target=https%3A%2F%2Fcoding.imooc.com%2Fclass%2F398.html%23Anchor "https://coding.imooc.com/class/398.html#Anchor")
- [万字长文 - Kotlin 协程进阶](https://juejin.cn/post/6950616789390721037#heading-17 "https://juejin.cn/post/6950616789390721037#heading-17")
- [一文快速入门 Kotlin 协程](https://juejin.cn/post/6908271959381901325#heading-2 "https://juejin.cn/post/6908271959381901325#heading-2")
- [最全面的Kotlin协程: Coroutine/Channel/Flow 以及实际应用](https://juejin.cn/post/6844904037586829320#heading-10 "https://juejin.cn/post/6844904037586829320#heading-10")
- [史上最详Android版kotlin协程入门进阶实战(二)](https://juejin.cn/post/6953287252373930021#heading-5 "https://juejin.cn/post/6953287252373930021#heading-5")